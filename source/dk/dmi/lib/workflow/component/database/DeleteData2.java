...
public class DeleteData2 {
	...
	@ExecuteMethod(
			argumentDisplayTypes = {WorkflowAnnotations.ARGUMENT_DISPLAY_TYPE_LIST, WorkflowAnnotations.ARGUMENT_DISPLAY_TYPE_LIST, WorkflowAnnotations.ARGUMENT_DISPLAY_TYPE_CHECK_BOX, WorkflowAnnotations.ARGUMENT_DISPLAY_TYPE_SQL_AREA_SMALL},
			argumentDescriptions = {"Choose database", "Choose entity", "Should context objects automatically be adjusted to sql syntax based on data type", "SQL where clause (without specifying WHERE)"})
	public void execute(String databaseName, String entityName, boolean autoDetectDataTypes, String whereClause) throws ClassNotFoundException, InstantiationException, IllegalAccessException, NoSuchMethodException, SecurityException, IllegalArgumentException, InvocationTargetException {
		...
		CommonDataUtils.deleteByWhereClause(entityManager, entityUtils.getDatabaseSchema(), entityUtils.getEntityPackagePath()+entityName, autoDetectDataTypes, whereClause, workflowContextController.getWorkflowContextMap());
