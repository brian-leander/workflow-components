...
public class SelectData2 {
	...
	@ExecuteMethod(
			argumentDisplayTypes = {WorkflowAnnotations.ARGUMENT_DISPLAY_TYPE_LIST, WorkflowAnnotations.ARGUMENT_DISPLAY_TYPE_LIST, WorkflowAnnotations.ARGUMENT_DISPLAY_TYPE_CHECK_BOX, WorkflowAnnotations.ARGUMENT_DISPLAY_TYPE_SQL_AREA_SMALL, WorkflowAnnotations.ARGUMENT_DISPLAY_TYPE_TEXT},
			argumentDescriptions = {"Choose database", "Choose entity", "Should context objects automatically be adjusted to sql syntax based on data type", "SQL where clause (without specifying WHERE)", "Maximum number of results to return (use null to ignore)"}, 
			returnDescription = "List of persistence objects specified by entity type")
	public List<?> execute(String databaseName, String entityName, boolean autoDetectDataTypes, String whereClause, Integer maxResults) throws ClassNotFoundException, InstantiationException, IllegalAccessException, NoSuchMethodException, SecurityException, IllegalArgumentException, InvocationTargetException {
		...
		List<?> persistenceObjectList = CommonDataUtils.selectEntityListByWhereClause(entityManager, entityUtils.getDatabaseSchema(), entityUtils.getEntityPackagePath()+entityName, autoDetectDataTypes, whereClause, maxResultsInt, workflowContextController.getWorkflowContextMap());
