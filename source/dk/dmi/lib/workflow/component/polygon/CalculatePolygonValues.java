...
@Component(
        ...
        version = 1)
public class CalculatePolygonValues extends BaseComponent {
	...
	@ExecuteMethod(
			argumentDescriptions = {"Location of the interpolation folder for the parameter grid value files, e.g. (Y:\\\\) or (\\\\vortex.dmi.dk\\tkdata\\)", "Polygon type to calculate values for", 
					...
					"Start date of calculation interval", "End date of calculation interval", "Commit interval between grid files (use null for never)", "Email to send progress noptifications to (use null to ignore)"})
    public void execute(String location, PolygonType polygonType, Parameter parameter, Date beginDateTime, Date endDateTime, Long commitInterval, String notificationEmail) throws Exception {
		...
		if(parameter == null) {
			calculatePolygonValuesForAllPolygonTypeParameters(location, polygonType, beginDateTime, endDateTime, polygonValueCalculator, polygonTypeParameters, commitInterval, notificationEmail);
		} else if(polygonTypeParametersContainParameter(polygonTypeParameters, parameter)) {
			polygonValueCalculator.calculatePolygonValuesForParameterInDateTimeInterval(location, polygonType, parameter, beginDateTime, endDateTime, commitInterval, notificationEmail, false);
		} else {
			throw new Exception("Wrong parameter is passed in component CalculatePolygonValues");
		}
    }

	void calculatePolygonValuesForAllPolygonTypeParameters(String drive, PolygonType polygonType, Date beginDateTime, Date endDateTime, PolygonValueCalculator polygonValueCalculator, List<PolygonTypeParameter> polygonTypeParameters, Long commitInterval, String notificationEmail) throws Exception {
		for (PolygonTypeParameter polygonTypeParameter : polygonTypeParameters) {
			...
			polygonValueCalculator.calculatePolygonValuesForParameterInDateTimeInterval(drive, polygonType, currentParameter, beginDateTime, endDateTime, commitInterval, notificationEmail, false);
		}
	}
