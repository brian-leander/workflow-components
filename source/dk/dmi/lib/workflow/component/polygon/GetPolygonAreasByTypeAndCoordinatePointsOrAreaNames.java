...
@Component(
		name = "Get polygon areas by type and coordinate points or area names", 
		category = "Polygon",
		description = "Get the polygon areas for a given polygon type and assosiated polygon area names or point list. The point list should include a list of eastings and northings coordinate points where x = eastings and y = northings. "
        ...
        version = 1)
public class GetPolygonAreasByTypeAndCoordinatePointsOrAreaNames extends BaseComponent {
	...
	@ExecuteMethod(
			argumentDescriptions = {"The polygon type to filter by", "Filter by loist of Point2D.Double repesenting eastings northings coordinates (use null to ignore)", "Filter by list of polygon area names (use null to ignore)"},
			returnDescription = "A list of polygon areas, covering the coordinates")
    public List<PolygonArea> execute(PolygonType polygonType, List<Point2D> eastingsNorthingsPoints, List<String> polygonAreaNames) {
		List<PolygonArea> polygonAreaList = null;
		
		if(eastingsNorthingsPoints != null) {
			if(eastingsNorthingsPoints.size() < 20) {
				polygonAreaList = getPolygonAreaMapByTypeAndCoordinatePoints(polygonType, eastingsNorthingsPoints, polygonAreaNames);
			} else {
				polygonAreaList = getPolygonAreaMapByTypeAndCoordinatePointsManyPoints(polygonType, eastingsNorthingsPoints, polygonAreaNames);
			}
			
			tryAddMissingPoints(polygonType, polygonAreaList, eastingsNorthingsPoints, polygonAreaNames);
		} else {
			polygonAreaList = getPolygonAreaMapByType(polygonType, polygonAreaNames);
		}
	...
	List<PolygonArea> getPolygonAreaMapByType(PolygonType polygonType, List<String> polygonAreaNames) {
		PolygonAreaController polygonAreaController = PolygonAreaController.getInstance();
		List<PolygonArea> polygonAreaList = null;
		
		if(polygonAreaNames == null) {
			polygonAreaList = polygonAreaController.getPolygonAreasByType(polygonType);
		} else {
			polygonAreaList = polygonAreaController.getPolygonAreaByTypeAndNames(polygonType, polygonAreaNames);
		}
	...
	List<PolygonArea> getPolygonAreaMapByTypeAndCoordinatePoints(PolygonType polygonType, List<Point2D> eastingsNorthingsPoints, List<String> polygonAreaNames) {
		...
		for (Point2D eastingsNorthingsPoint : eastingsNorthingsPoints) {
			...
			for (PolygonArea currentPolygonArea : currentPolygonAreaList) {
				addPolygonAreaAndPoint(currentPolygonArea, polygonAreaList, eastingsNorthingsPoint, polygonAreaCoordinateMap, polygonAreaNames);
			}
		}
	...
	void addPolygonAreaAndPoint(PolygonArea currentPolygonArea, List<PolygonArea> polygonAreaList, Point2D eastingsNorthingsPoint, Map<Long, List<Point2D>> polygonAreaCoordinateMap, List<String> polygonAreaNames) {
		if(polygonAreaNames == null || doesPolygonAreaNameListContainsName(polygonAreaNames, currentPolygonArea.getName())) {
	...
	List<PolygonArea> getPolygonAreaMapByTypeAndCoordinatePointsManyPoints(PolygonType polygonType, List<Point2D> eastingsNorthingsPoints, List<String> polygonAreaNames) {
	...
	void tryAddMissingPoints(PolygonType polygonType, List<PolygonArea> polygonAreaList, List<Point2D> eastingsNorthingsPoints, List<String> polygonAreaNames) {
		...
		for (Point2D point : eastingsNorthingsPoints) {
			...
			if(isPointMissing) {
				...
				if(nearestPolygonArea != null) {
					addPolygonAreaAndPoint(nearestPolygonArea, polygonAreaList, point, polygonAreaCoordinateMap, polygonAreaNames);
				}
			}
		}
	}
	...
	boolean doesPolygonAreaNameListContainsName(List<String> polygonAreaNames, String polygonAreaName) {
		for (String currentPolygonAreaName : polygonAreaNames) {
	        if (currentPolygonAreaName.equalsIgnoreCase(polygonAreaName)) {
	            return true;
	        }
	    }
