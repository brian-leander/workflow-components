...
@WorkflowAnnotations.Component(
        name = "Parse large ZRX file",
        category = "Asiaq",
        description = "Parse file.",
        version = 1)
public class ParseZrxHistoricalData extends BaseComponent {
    ...
    @WorkflowAnnotations.ExecuteMethod(
            argumentDescriptions = {"List of String each containing line of observation data file in ZRX format."},
            returnDescription = "List of type (dk.dmi.lib.synop.asiaq.ObservationWithType, parameter type).")
    public List<ObservationWithType> parseZrxFile(List<String> zrxObservation) {
        return parseZrxFormat(zrxObservation);
    }

    public List<ObservationWithType> parseZrxFormat(List<String> zrxObservation) {
        List<ObservationWithType> observations = new ArrayList<>(6000000);

    //        String[] lines = zrxObservation.split(System.lineSeparator());
    //        int lineNo = 0;
    //        while (lineNo < lines.length) {

        for (int i = 0; i < zrxObservation.size(); i++) {
            StringBuilder header = new StringBuilder();
            // Read header lines

            while (i < zrxObservation.size() && zrxObservation.get(i).startsWith(HEADER_PREFIX)) {
                String headerLine = zrxObservation.get(i);
                header.append(headerLine.replace(HEADER_PREFIX, "").trim());
                i++;
            }
...
//                while (lineNo < lines.length && !lines[lineNo].startsWith(HEADER_PREFIX)) {
                while (i < zrxObservation.size() && !zrxObservation.get(i).startsWith(HEADER_PREFIX)) {
                    String[] observationRecords = zrxObservation.get(i).trim().split(" ");
